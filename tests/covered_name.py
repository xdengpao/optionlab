...
def calculate_strategy_inputs(stock_name, stock_price, stock_n, option_stock_price, option_price, option_n, volatility, start_date, target_date, interest_rate):
    ...
    strategy = [
        {"type": "stock", "n": stock_n, "action": "buy"},
        {"type": "call", "strike": option_stock_price, "premium": option_price, "n": option_n, "action": "sell"},
    ]
...
def plot_strategy(out, stock_name, stock_price):
    """
    ...
    for low, high in out.profit_ranges:
        plt.title(
            ...
            f"Profitable stock price range:  {low:.2f} ---> {high:.2f}"
        )
...
def main() -> None:
    ...
    plot_strategy(out, stock_name,stock_price)
    display_results(out)
